...
function gridInputClick(event, sort = true, button = null) {
    ...
    let options = optionSets[cat].filter(option => {
        switch (cat) {
            ...
            case 'bullet':
                return button.dataset.bulletType == bullets[option.metatags[0]].type;
            default: return true;
        }
    });
    if (sort) {
...
function removeBulletDiv() {
    if (bulletDiv = document.querySelector("#bullets")) {
        while (bulletDiv.firstChild) {
            delete teamData[bulletDiv.firstChild.id];
            bulletDiv.firstChild.remove();
        }
        bulletDiv.remove();
    }
}
...
function setButtonToItem(button, optionSet, selectedOption, uncap = null, options = {}) {
    ...
    switch (optionSet) {
        ...
        case 'weapons':
            ...
            break;
        case 'summons':
            if (button.parentElement.classList.contains("team-summon")) {
                uncap = addUncapButton(button, optionSet, selectedOption, uncap, id, options.trans);
            }
            else {
                uncap = addSummonUncapButtons(button, id, uncap, options.trans);
                addQuickSummonButton(button);
            }
...
function setButtonBackground(button, selectedOption, optionSet, uncap, id) {
    ...
    switch (optionSet) {
        ...
        case 'shield':
            ...
            break;
        case 'bullet':
            backgroundUrl = `url(https://raw.githubusercontent.com/cajunwildcat/The-GrandCypher/main/assets/gear/bullets/${id}.webp)`
            break;
        case 'opusSkill2':
        case 'opusSkill3':
        case 'ultimaSkill1':
    ...
    if ((optionSet != "characters" && maxUncap == 4) || maxUncap == 5) {
        ...
        uncapButton.onclick = (e) => {
            ...
            switch (optionSet) {
                ...
                case "weapons": uncap = uncapButton.dataset.toggled === "true" ? weapons[id].maxUncap : 0; break;
                case "summons": uncap = uncapButton.dataset.toggled === "true" ? summons[id].maxUncap : 0; break;
            }
...
function addSummonUncapButtons(button, id, uncap, trans) {
    ...
    teamData[button.id + "Uncap"] = uncap;
    let uncapDiv = document.createElement("div");
    ...
    for (let i = 1; i <= Math.min(5, maxUncap); i++) {
        ...
        else if (i <= 5) star.classList.add("blue-star");
        star.title = `${i}★`;
        star.dataset.uncap = i;
        star.dataset.toggled = uncap >= i;
        star.onclick = (e) => {
            e.stopPropagation();
            let newUncap;
            if (teamData[button.id + "Uncap"] == i) newUncap = i - 1;
            else newUncap = i;
            starClick(newUncap);
        }
    ...
    if (maxUncap == 6) {
        ...
        star.title = "6★";
        star.dataset.trans = trans;
        star.onclick = openTransDropdown;
        function openTransDropdown(e) {
            e.stopPropagation();
            hideDropdown();
            star.onclick = closeTransDropdown;
            let dropdown = document.createElement("div");
            dropdown.classList.add("dropdown");
            dropdown.id = "uncap-dropdown";
            dropdown.innerHTML = `<ul id='options-list'>
            ...
            dropdown.querySelectorAll("li").forEach(li => {
                li.onclick = (e) => {
                    ...
                    if (newUncap != "t0") {
                        ...
                        teamData[button.id + "Trans"] = newUncap;
                    }
                    else {
                        teamData[button.id + "Uncap"] = 5;
                        newUncap = 5;
                        delete teamData[button.id + "Trans"];
                    }
...
function addBulletButtons(weaponID) {
    let bullets = weapons[weaponID].bullets;
    let mh = document.querySelector(".w-grid-main");
    let bulletDiv = document.createElement("div");
    bulletDiv.classList.add("grid-input");
    //bulletDiv.classList.add("class-gear");
    bulletDiv.id = "bullets";
    for (let i = 0; i < bullets.length; i++) {
        if (!bullets[i]) continue;
        let bulletButton = document.createElement("button");
        bulletButton.id = `bullet${i + 1}`;
        bulletButton.dataset.options = "bullet";
        bulletButton.dataset.bulletType = bullets[i];
        bulletDiv.appendChild(bulletButton);
        bulletButton.onclick = gridInputClick;
        bulletButton.oncontextmenu = gridInputContextMenu;
    }
    mh.appendChild(bulletDiv);
}
...
|rotation=
    ...
    function getBulletInfo() {
        if (!Object.keys(teamData).some(k => k.includes("bullet"))) return "";
        let equippedBullets = [];
        for (let i = 1; i < 7; i++) {
            if (teamData[`bullet${i}`]) equippedBullets.push(aliases[teamData[`bullet${i}`]][0]);
        }
        return `|bullets=${equippedBullets.join(",")}`;
    }
...
function importWikiTextV2(inputData) {
    ...
    if (data.class == "Soldier" && data.bullets) {
        ...
        for (let i = 0; i < bullets.length; i++) {
            if (bullets[i] == "") continue;
            setGridData(`bullet${i + 1}`, bullets[i]);
        }
    }
    //summons
...
function exportURL() {
    ...
    if (Object.keys(teamData).some(k => k.includes("bullet"))) {
        let equippedBullets = [];
        for (let i = 1; i < 7; i++) {
            if (teamData[`bullet${i}`]) equippedBullets.push(Object.keys(bullets).find(k => bullets[k].name == teamData[`bullet${i}`]));
            else equippedBullets.push("");
        }
        while (equippedBullets.length && equippedBullets.at(-1) === "") equippedBullets.pop();
        if (equippedBullets.length > 0) wskill.push(`&bullet=${equippedBullets.join(",")}`)
    }
